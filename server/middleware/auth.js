...
export const authorizeRoles = (...roles) => {
    return (req, res, next) => {
        if (!req.user || !req.user.role) { // Check if user or role exists
            console.warn('User or role is undefined. Authentication may have failed.');
            return res.status(403).json({ message: 'Access denied.  User role not found.' });
        }

        if (!roles.includes(req.user.role)) {
            console.log(`User role ${req.user.role} not authorized for this endpoint. Required roles: ${roles.join(', ')}`);
