    ...
    try {
        ...
        for (const item of approachingResult.rows) {
            try {
                ...
                console.error(`Error creating alert for approaching deadline for item ${item.id}:`, error);
            }
        }
        ...
        for (const item of overdueResult.rows) {
            try {
                ...
                console.error(`Error creating alert for overdue item ${item.id}:`, error);
            }
        }
...
const sendDeadlineEmail = async (type, item) => { //Expect 'item' object now
    try {
